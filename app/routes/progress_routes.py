...
def get_attempt_details(
    ...
    # Fetch question attempts details
    q_attempts = (
    ...
    for qa in q_attempts:
        q = db.query(Question).filter(Question.id == qa.question_id).first()
        if not q:
            continue
    ...
    # Group correct vs incorrect by subject
    subject_accuracy = {}
    for qa in q_attempts:
        q = db.query(Question).filter(Question.id == qa.question_id).first()
        if not q:
        ...
        chapter = db.query(Chapter).filter(Chapter.id == q.chapter_id).first()
        if chapter:
...
def get_user_bookmarks(
    ...
    results = []
    for b in bookmarks:
        q = db.query(Question).filter(Question.id == b.question_id).first()
        if not q:
            continue
        results.append({
            "bookmark_id": b.id,
